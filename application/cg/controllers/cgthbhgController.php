...
/**********************************************************
 * 模块：    采购模块(CG)
 * 机能：    采购退货(不合格品)(CGTHBHG)
 * 作成者：刘枞
 * 作成日：2011/08/05
 * 更新履历：
 **********************************************************/

class cg_cgthbhgController extends cg_controllers_baseController {
	...
	public function thdlistAction(){
		...
		$this->_view->display ( "cgthbhg_02.php" );
	}
		...
		if($this->_getParam ( "isfilter", '0' )=='0'){
		   //取得一般查询条件参数并保存至session
			if($this->_request->isPost()){
				$_SESSION['cg_cgthbhg_searchParams'] = $_POST;
				unset($_SESSION['cg_cgthbhg_filterParams']);       //清空精确查询条件
			}
			
		}else{//精确查询
			//取得过滤条件参数并保存至session
			if($this->_request->isPost()){
				$_SESSION['cg_cgthbhg_filterParams'] = $_POST;
				unset($_SESSION['cg_cgthbhg_searchParams']);                      //清空一般查询条件
			}
		}
		
		//取得检索条件
		$filter['filterParams'] = $_SESSION['cg_cgthbhg_filterParams'];           //精确查询条件
		$filter['searchParams'] = $_SESSION['cg_cgthbhg_searchParams'];           //固定查询条件
		
		$model = new cg_models_cgthbhg();
	...
	public function thmxlistAction() {
		...
		$this->_view->display ( "cgthbhg_03.php" );
	}
