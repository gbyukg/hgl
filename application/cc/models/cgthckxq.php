...
class cc_models_cgthckxq extends Common_Model_Base {
	...
	/**
	 * 根据采购退货出库单编号取得采购退货出库单信息
	 * 
	 * @param array $filter
	 * @return array
	 */
	public function getCgthckInfo($ckdbh){
		//检索SQL
		$sql = "SELECT TO_CHAR(A.KPRQ,'yyyy-mm-dd') AS KPRQ,A.CHKDBH,A.CKDBH,C.BMMCH,B.YGXM,A.DWBH,"
			  ...
			  ."AND A.CHKDBH = :CHKDBH "     //采购退货出库单编号
			  ."AND A.CHKLX = '2' ";         //出库类型      固定值：‘2’退货出库
	...
	public function getMingxiData($filter){
		...
		$sql = "SELECT "          
			  ...
			  ."WHERE A.QYBH = :QYBH "       //区域编号
			  ."AND A.CHKDBH = :CHKDBH ";    //采购退货单编号  例：CGT10121300001
