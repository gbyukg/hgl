...
/******************************************************************
 ***** 模         块：       仓储模块(CC)
 ***** 机         能：        库间调拨入库详情(KJDBRKXQ)
 ***** 作  成  者：        刘枞
 ***** 作  成  日：        2011/01/26
 ***** 更新履历：
 ******************************************************************/

class cc_models_kjdbrkxq extends Common_Model_Base {
	/**
	 * 库间调拨入库单信息获取
	 *
	 * @param string $bh
	...
	function getinfoData($bh){
		...
		$sql = "SELECT TO_CHAR(A.KPRQ,'YYYY-MM-DD') AS KPRQ,"           //开票日期
				...
				."A.DCHCK,"  		      //调出仓库编号
				."A.DRCK,"                //调入仓库编号
				."TO_CHAR(A.BGRQ,'YYYY-MM-DD HH24:mi:ss') AS BGRQ "    //变更日期
			  ."FROM H01DB012412 A "
	...
	public function getmingxi($filter) {
		...
		$sql = "SELECT A.XUHAO,"          //序号
			  ...
			  ."LEFT JOIN H01DB012402 G ON A.QYBH = G.QYBH AND G.CKBH = :CKBH AND A.DRKQ = G.KQBH "
			  ."LEFT JOIN H01DB012403 H ON A.QYBH = H.QYBH AND H.CKBH = :CKBH AND A.DRKQ = H.KQBH AND A.DRKW = H.KWBH "
			  ."WHERE A.QYBH = :QYBH "
			  ."AND A.DJBH = :DJBH "
			  ."ORDER BY A.XUHAO";
