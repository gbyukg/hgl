...
class cc_models_cgthck extends Common_Model_Base {
	...
	private $idx_BZHDWBH = 19;        // 包装单位编号
	private $idx_TYMCH=20;            // 通用名称
	private $idx_JLGG=21;             // 计量规格
	private $idx_CKBH=22;             // 仓库编号
	private $idx_KQBH=23;             // 库区编号
	private $idx_KWBH=24;             // 库位编号
	...
	public function getGridDanjuData($filter) {
		...
		$sql = "SELECT CGTHDBH,TO_CHAR(KPRQ,'YYYY-MM-DD') AS KPRQ,DWBH,DWMCH,YRKDBH,TO_CHAR(YRKDRQ,'YYYY-MM-DD') AS YRKDRQ,BMMCH,YWYXM,QYBH,THDZHT,QXBZH FROM "
				...
				."WHERE QYBH = :QYBH AND THDZHT = '0' AND QXBZH != 'X' AND SHHZHT = '1' AND THLX = '1' ";
	 ...
	 * 出库单保存处理
	 */
	public function updateKucun($chkdbh) {
		$result ['status'] = '0';
		...
		foreach ( $_POST ["#grid_mingxi"] as $row ) {
			...
			//取得即时库存信息
			$sql = "SELECT A.QYBH,A.CKBH,A.KQBH,A.KWBH,A.SHPBH,A.PIHAO,A.RKDBH,A.ZKZHT,".
				   "A.BZHDWBH,A.SHULIANG,".
...
//			}
			...
			if($result['status']=='0'){
				//更新在库和移动履历信息
			    $this->updateZaiku( $row, $recs, $chkdbh );
			}
		}
	 ...
	 * 更新在库和移动履历信息
	 */
	public function updateZaiku($row,$kucun,$chkdbh) {
		...
		$bind ['KQBH'] = $kucun ['KQBH'];                    //库区编号
		$bind ['KWBH'] = $kucun ['KWBH'];                    //库位编号
		...
		$lvli ["SHULIANG"] = $shuliang_TH * - 1;                 //移动数量
		$lvli ["ZHYZHL"] = '12';                                 //转移种类 [12:入库取消]
