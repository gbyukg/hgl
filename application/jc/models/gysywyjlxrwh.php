 ...
 * 更新履历：
 *********************************/
class jc_models_gysywyjlxrwh extends Common_Model_Base {
	 ...
	 */
		public function getshpinGridData($dwbh){
			...
			$sql =" SELECT LXRXM,SSBMMCH,YOUXIANJI,DHHM,SHJHM,DZYJ,XUHAO FROM H01DB012111 WHERE QYBH =:QYBH AND  LXRQF ='C' AND DWBH =:DWBH ORDER BY XUHAO";
			$bind ['QYBH'] = $_SESSION ['auth']->qybh;
			$bind ['DWBH'] = $dwbh;
	 ...
	 */
		public function getdanweiGridData($dwbh){
			...
			$sql = " SELECT A.YGBH,B.YGXM,B.SSBM,B.ZHJM,A.YOUXIANJI,B.DHHM,B.SHJHM,B.DZYJ FROM H01DB012110 A LEFT JOIN H01DB012113 B ".
				   " ON A.QYBH = B.QYBH AND A.YGBH = B.YGBH WHERE A.QYBH =:QYBH AND A.DWBH =:DWBH AND A.YGQF ='C' ORDER BY A.YGBH";
			$bind ['QYBH'] = $_SESSION ['auth']->qybh;
			$bind ['DWBH'] = $dwbh;
			$recs = $this->_db->fetchAll($sql,$bind);
		    return Common_Tool::createXml ( $recs, true );
	 ...
	 */
		public function delectShpbh($dwbh){
			...
			$sql = " DELETE  FROM H01DB012110 WHERE QYBH =:QYBH AND DWBH =:DWBH AND YGQF ='C'";
			$bind ['QYBH'] = $_SESSION ['auth']->qybh;
			$bind ['DWBH'] = $dwbh;
	 ...
	 */
		...
		public function delectDwbh($dwbh){
			...
			$sql = " DELETE  FROM H01DB012111 WHERE QYBH =:QYBH AND DWBH =:DWBH AND LXRQF ='C'";
			$bind ['QYBH'] = $_SESSION ['auth']->qybh;
			$bind ['DWBH'] = $dwbh;
			return $this->_db->query( $sql, $bind );
